...
sampleTypes<-function(df, MS.Samples, LD1.Samples, LD2.Samples, Travel.Blanks, Field.Blanks){
  ...
  df$Sample.Type<-if_else(str_detect(df$Sample.Name, "(?i)CCV"), "CLC", df$Sample.Type)
  df$Sample.Type<-if_else(str_detect(df$Sample.Name, "(?i)ISC"), "LVM", df$Sample.Type)
  df$Sample.Type<-if_else(str_detect(df$Sample.Name, "(?i)cal"), "CAL", df$Sample.Type)
  df$Sample.Type<-if_else(str_detect(df$Sample.Name, "(?i)IB"), "CLB", df$Sample.Type)
  df$Sample.Type<-if_else(str_detect(df$Sample.Name, "(?i)MB"), "LMB", df$Sample.Type)
  df$Sample.Type<-if_else(str_detect(df$Sample.Name, "(?i)OPR"), "OPR", df$Sample.Type)
